...
resource "aws_iam_role_policy_attachment" "AmazonEKSCNI_Policy" {
...
resource "aws_iam_role_policy_attachment" "AmazonEBSCSIDriverPolicy2" {
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonEBSCSIDriverPolicy"
  role       = aws_iam_role.eksWorkerRole.name
}
...
resource "aws_iam_instance_profile" "eksWorkerRole" {
  depends_on = [aws_iam_role.eksWorkerRole]
  name       = "eksWorkerRole-new-profile"
  role       = aws_iam_role.eksWorkerRole.name
}