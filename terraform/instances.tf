...
resource "aws_instance" "bastion_server" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "aws elb modify-load-balancer-attributes --load-balancer-name ${data.aws_elb.aws-hfs-lb.name} --load-balancer-attributes ${jsonencode({"ConnectionSettings":{"IdleTimeout":1200}})}",
